...
export default function ChatWidget() {
    ...
    const [view, setView] = useState<'menu' | 'chat' | 'call'>('menu');
    const [messages, setMessages] = useState<Message[]>([
        { id: '1', text: "Hi! I'm PrimeCare's AI assistant. How can I help you today?", sender: 'ai', timestamp: new Date() }
    ]);
    ...
    const sendMessage = async () => {
        ...
        try {
            // Simulate AI response (connect to your n8n/AI backend)
            await new Promise(resolve => setTimeout(resolve, 1000));

            // AI response based on keywords
            let aiResponse = "Thank you for your message. One of our care coordinators will get back to you shortly. Is there anything specific I can help you with?";

            const lowerText = inputText.toLowerCase();
            if (lowerText.includes('price') || lowerText.includes('cost') || lowerText.includes('rate')) {
                aiResponse = "Our rates vary based on service type. PSW home care starts at $28/hour. Foot care assessments are $75. Would you like me to connect you with our team for a detailed quote?";
            } else if (lowerText.includes('book') || lowerText.includes('appointment')) {
                aiResponse = "I'd be happy to help you book an appointment! You can call us at (416) 555-1234 or I can have someone call you back. What service are you interested in?";
                ...
                aiResponse = "Our Personal Support Workers provide bathing assistance, meal prep, mobility support, and companionship. We offer flexible scheduling from 4 hours to 24/7 care. How can I help you further?";
            }
    ...
    const startCall = async () => {
        ...
        try {
            ...
            // In production, this would connect to WebRTC or voice API
            // For now, show demo state
            setTimeout(() => {
                setCallStatus('AI: Hello! Welcome to PrimeCare. How can I assist you today?');
            }, 3000);
        } catch (error) {
                ...
                <div style={{
                    ...
                    {view === 'menu' && (
                        <div style={{ padding: '1rem' }}>
                            <button onClick={() => setView('chat')} style={{
                                ...
                                backgroundColor: '#e8f5e9', borderRadius: '12px', border: 'none', cursor: 'pointer', marginBottom: '0.75rem',
                                textAlign: 'left',
                            }}>
                            ...
                            <button onClick={() => setView('call')} style={{
                                display: 'flex', alignItems: 'center', gap: '0.75rem', padding: '1rem', width: '100%',
                                backgroundColor: '#e3f2fd', borderRadius: '12px', border: 'none', cursor: 'pointer', marginBottom: '0.75rem',
                                textAlign: 'left',
                            }}>
                            ...
                            <div style={{ borderTop: '1px solid #eee', paddingTop: '1rem', marginTop: '0.5rem' }}>
                                ...
                                <div style={{ display: 'flex', gap: '0.5rem' }}>
                                    <a href="tel:+14165551234" style={{
                                        ...
                                        padding: '0.75rem', backgroundColor: '#f8f9fa', borderRadius: '8px', textDecoration: 'none', color: '#333', fontSize: '0.85rem',
                                    }}>
                                        📞 Call
                                    </a>
                                    <a href="https://wa.me/14165551234" target="_blank" rel="noopener noreferrer" style={{
                                        flex: 1, display: 'flex', alignItems: 'center', justifyContent: 'center', gap: '0.5rem',
                                        padding: '0.75rem', backgroundColor: '#25D366', borderRadius: '8px', textDecoration: 'none', color: 'white', fontSize: '0.85rem',
                                    }}>
                                        💬 WhatsApp
                                    </a>
                        ...
                        <>
                            <div style={{ flex: 1, overflowY: 'auto', padding: '1rem', backgroundColor: '#f8f9fa' }}>
                                ...
                                {isLoading && (
                                    <div style={{ display: 'flex', justifyContent: 'flex-start', marginBottom: '0.75rem' }}>
                                        <div style={{ padding: '0.75rem 1rem', backgroundColor: 'white', borderRadius: '16px', boxShadow: '0 1px 2px rgba(0,0,0,0.1)' }}>
                            ...
                            <div style={{ padding: '1rem', borderTop: '1px solid #eee', display: 'flex', gap: '0.5rem' }}>
                                <input
                                    ...
                                    style={{
                                        flex: 1, padding: '0.75rem', borderRadius: '24px', border: '1px solid #ddd',
                                ...
                                <button
                                    ...
                                    style={{
                                        padding: '0.75rem 1rem', backgroundColor: '#00897b', color: 'white',
                            ...
                            {!isCallActive ? (
                                <>
                                    ...
                                    <button
                                        onClick={startCall}
                                        style={{
                                            padding: '1rem 2rem', backgroundColor: '#00897b', color: 'white',
                                ...
                                <>
                                    <div style={{
                                        ...
                                        backgroundColor: '#e8f5e9', margin: '0 auto 1rem',
                                        display: 'flex', alignItems: 'center', justifyContent: 'center',
                                        animation: 'pulse 2s infinite',
                                    }}>
                                        <span style={{ fontSize: '3rem' }}>🎙️</span>
                                    </div>
                                    <p style={{ color: '#00897b', fontWeight: 'bold', marginBottom: '1rem' }}>
                                        {callStatus}
                                    </p>
                                    <button
                                        onClick={endCall}
                                        style={{
                                            padding: '1rem 2rem', backgroundColor: '#f44336', color: 'white',
                                            border: 'none', borderRadius: '50px', cursor: 'pointer', fontSize: '1rem',
                                        }}
                                    >
                                        End Call
                    ...
                    {view === 'menu' && (
                        <div style={{ padding: '0.75rem', borderTop: '1px solid #eee', textAlign: 'center' }}>
                            <Link to={RouteRegistry.CONTACT} onClick={() => setIsOpen(false)} style={{ color: '#00897b', textDecoration: 'none', fontWeight: 'bold', fontSize: '0.85rem' }}>
